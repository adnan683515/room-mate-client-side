...
const Login = () => {
    ...
    const cng  = mode ? 'text-white':'text-black'
    ...
    return (
        <div className='py-20 -mb-15'>
            <div className='flex lg:justify-around lg:flex-row flex-col'>
                ...
                <div className={`md:w-[50%] rounded-sm sm:[70%]  w-[98%] mx-auto sm:mt-0 mt-10  duration-700  border border-teal-200 p-8  space-y-3  ${mode ? "bg-black" : "bg-gray-200 "} dark:text-gray-800 `}>
                    ...
                    <form onSubmit={handleLoginFrom} className="space-y-6 sm:mt-0 mt-10  ">
                        <div className="space-y-1 text-sm">
                            <label htmlFor="username" className={`block ${cng}`}>Your Email</label>
                            <input required type="email" name="email" id="email" placeholder="Your Email" className={`
                                w-full px-4 py-3 rounded-md border-gray-300 bg-white ${mode ? "text-black" : "text-black"} border-fuchsia-600`} />
                        </div>
                        <div className="space-y-1 text-sm">
                            <label htmlFor="password" className={`block ${cng}`}>Password</label>
                            <input required type={show ? 'text' : 'password'} name="password" id="password" placeholder="Password" className="w-full px-4 py-3 rounded-md dark:border-gray-300 bg-white text-black " />
                            ...
                            <div className='flex gap-1'>
                                ...
                                <span className={`${cng}`}>show password</span>
                            </div>
                        </div>
                    ...
                    <p className={`text-xs text-center sm:px-6 ${mode ? 'text-white':'text-gray-600'} `}>Don't have an account?
                        <Link to={'/signup'} rel="noopener noreferrer" className={`underline ${mode ?'text-white':'text-gray-800'}`}>Sign up</Link>
                    </p>
                </div>
            </div>
        </div>
